...
const SUBMISSION_STATUSES: any[] = [
  { value: "completed", label: "Completed", icon: <CheckCircle2 className="w-4 h-4" />, className: "bg-emerald-500 hover:bg-emerald-600" },
...
export const AssignmentSubmissionsModal: React.FC<AssignmentSubmissionsModalProps> = ({
  ...
  // 1. Get all students enrolled in this lecture's course
  const { data: students, isLoading: studentsLoading, isError: studentsError } = useLectureStudentsInfoQuery(lectureId || "");
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl max-h-[85vh] flex flex-col p-0 overflow-hidden rounded-3xl border-none shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6 min-h-0 bg-white">
          {(studentsLoading || submissionsLoading) ? (
            <div className="h-60 flex flex-col items-center justify-center gap-4">
              <Loader2 className="w-10 h-10 animate-spin text-primary/40" />
              <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Loading Submissions...</p>
            </div>
          ) : (studentsError || !lectureId) ? (
            <div className="h-40 flex items-center justify-center text-rose-500 font-bold bg-rose-50 rounded-2xl">
              Failed to load required data
            </div>
          ) : students?.length === 0 ? (
            <div className="h-40 flex flex-col items-center justify-center text-slate-400 gap-2">
              <XCircle className="size-8 opacity-20" />
              <p className="font-bold">No students found for this assignment</p>
            </div>
          ) : (
            <div className="space-y-4">
              {students?.map((student) => {
                const submission = submissions.find((s: any) => s.studentId === student.studentId);
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md">Roll: {student.rollNo}</span>
                          {!submission && (
