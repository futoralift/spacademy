...
export const TestSubmissionsModal: React.FC<TestSubmissionsModalProps> = ({
  ...
  const { data: allStudentsData, isLoading: studentsLoading } = useStudentsQuery({
    limit: 15
  });
  ...
  const allStudents = (allStudentsData as any)?.data ?? [];
  ...
  const onlineResults = rawAnswers.reduce((acc: any, sa: any) => {
    if (!acc[sa.studentId]) {
      acc[sa.studentId] = {
        ...
        studentName: sa.studentName || "Unknown Student",
        answers: [],
        totalScore: 0,
        maxScore: 0,
        correctCount: 0,
  ...
  const isLoading = testMode === 'online' ? (answersLoading || studentsLoading) : (studentsLoading || attemptsLoading);
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-5 min-h-0">
          {isLoading ? (
            ...
            sortedOnline.length === 0 ? (
              <div className="h-80 flex flex-col items-center justify-center rounded-4xl border">
                <div className="size-20 rounded-[30px] flex items-center justify-center">
                  <XCircle className="size-10 opacity-20" />
                </div>
                <div className="text-center">
                  <p className="font-semibold text-xl">No Submissions Yet</p>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-5">
                      ...
                      <div className="text-left">
                        <p className="font-black text-xl tracking-tight">{student.firstName} {student.lastName}</p>
                    ...
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        <p className="text-[10px] font-black text-slate-400 mb-2 px-1">Gained Marks</p>
