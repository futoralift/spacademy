...
const getFileVisual = (path: string | null | undefined, className?: string) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center size-full bg-slate-50 text-slate-400 gap-2 rounded border p-2 text-left">
        ...
        <div className="rounded-md border overflow-hidden text-left">
            ...
            >
                <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="flex flex-col gap-1 items-start min-w-0 text-left">
                ...
                <div className="border-t px-4 py-4 space-y-3 bg-muted/10">
                    ...
                    {q.explanation && (
                        <div className="space-y-2 border-l-2 border-primary/30 pl-3">
                            <p className="text-xs text-muted-foreground italic text-left">{q.explanation}</p>
                            {q.explanationImg && (
...
function AddQuestionForm({ testId, questionToEdit, onAdded, onCancel, disabled }: {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 rounded-md border p-4 bg-muted/10 text-left">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3">
                {OPTIONS.map(o => (
                    <div key={o.key} className="space-y-3 p-3 border rounded-md bg-white text-left">
...
export default function ManageQuestionsModal({ test, isOpen, onClose }: ManageQuestionsModalProps) {
    const { data: questionsData, refetch } = useQuestionsQuery({ testId: test?.id, limit: 15 })
    ...
    return (
        <Dialog open={isOpen} onOpenChange={open => { if (!open) { onClose(); setEditingQuestion(null); } }}>
            <DialogContent className="max-h-[90vh] flex flex-col sm:max-w-3xl text-left">
                <DialogHeader>
